...
class DemoNewWallet(StatesGroup):
    wallet = State()
    amount = State()


class DemoChangeWallet(StatesGroup):
    wallet = State()


class DemoChangePercentage(StatesGroup):
    percentage = State()
...
async def withdraw_main_menu(call: types.CallbackQuery):
    ...
    await call.message.answer_photo(photo=photo, caption=text, reply_markup=dm_inline.main_withdraw(language[4]))
...
async def change_wallet_new(call: types.CallbackQuery):
    ...
    await DemoChangeWallet.wallet.set()
...
async def change_wallet_step2(msg: types.Message, state: FSMContext):
    ...
    await msg.answer(text, reply_markup=dm_inline.main_withdraw(language[4]))
    await state.finish()
...
async def change_percentage(call: types.CallbackQuery):
    ...
    if language[4] == 'EN':
        text = "Wallet successfully updated!"
    await call.message.answer(text, reply_markup=dm_inline.withdraw_percentage(language[4]))
    await DemoChangePercentage.percentage.set()


async def change_percentage_step2(call: types.CallbackQuery, state: FSMContext):
    language = await users.user_data(call.from_user.id)
    await balance.update_percentage(call.from_user.id, int(call.data.split('_')[1]))
    await call.message.delete()
    text = "Процент реинвестирования успешно изменен!"
    if language[4] == 'EN':
        text = "Reinvestment percentage successfully updated!"
    await call.message.answer(text, reply_markup=dm_inline.main_withdraw(language[4]))
    await state.finish()
...
async def withdrawal_handler(call: types.CallbackQuery, state: FSMContext):
    ...
    if binance_balance:
        ...
        await call.message.answer(text, reply_markup=dm_inline.withdrawal_account(language[4]))
    else:
...
async def withdrawal_handler_personal(call: types.CallbackQuery, state: FSMContext):
    try:
        ...
        pass
    binance_balance = await dm_database.get_demo_balance(call.from_user.id)
    wallet = await users.user_data(call.from_user.id)
    language = await users.user_data(call.from_user.id)
    if wallet[6]:
        if binance_balance[6] >= 50:
            text = f"<b>Баланс, доступный к выводу:</b> {binance_balance[6]} USDT" \
                   f"\nCумма минимального вывода 50 USDT" \
                   f"\n\n💳 Напишите сумму USDT, которую хотите вывести:"
            if language[4] == "EN":
                text = f"The balance available for withdrawal: {binance_balance[6]} USDT" \
                       f"\nMinimum withdrawal amount is 50 USDT." \
                       f"\n\n💳 Please write the amount of USDT you want to withdraw:"
            del_msg = await call.message.answer(text, reply_markup=dm_inline.back_menu(language[4]))
            await state.set_state(DemoNewWallet.amount.state)
            ...
            await call.message.answer_photo(photo, text, reply_markup=dm_inline.main_withdraw(language[4]))
    else:
        photo = decouple.config("BANNER_WITHDRAWAL")
...
async def withdrawal_handler_collective(call: types.CallbackQuery, state: FSMContext):
    ...
    if first_trans:
        wallet = await users.user_data(call.from_user.id)
        if wallet[6]:
            balance_user = await dm_database.get_demo_balance(call.from_user.id)
            balance_user = balance_user[1]
            deposit = await dm_database.get_demo_balance(call.from_user.id)
            deposit = deposit[2]
            withdraw = await balance.get_balance(call.from_user.id)
            withdraw = withdraw[2]
            if balance_user > 0:
                if withdraw == 0:
                        ...
                        if first_trans[3] > 1000:
                            ...
                            if balance_user > 1000:
                                ...
                                await call.message.answer_photo(photo, text,
                                                                reply_markup=dm_inline.main_withdraw(language[4]))
                    elif date_first + datetime.timedelta(days=hold if hold is not None else 0) <= now:
                        text = f"<b>Баланс, доступный к выводу:</b> {balance_user} USDT" \
                               f"\nCумма минимального вывода 50 USDT" \
                               f"\n\n💳 Напишите сумму USDT, которую хотите вывести:"
                        if language[4] == 'EN':
...
async def handle_amount(msg: types.Message, state: FSMContext):
    ...
    if not msg.text.isdigit():
        ...
        personal_balance_user = await dm_database.get_demo_balance(msg.from_user.id)
        collective_balance_user = await dm_database.get_demo_balance(msg.from_user.id)
        async with state.proxy() as data:
            data['amount'] = msg.text
            if data.get("status") == "Личный":
                user_balance = personal_balance_user[6]
            else:
                user_balance = collective_balance_user[1]
            if user_balance >= int(msg.text):
                ...
                await msg.answer(text, reply_markup=dm_inline.finish_withdrawal(language[4]))
            else:
...
async def finish_withdrawal(call: types.CallbackQuery, state: FSMContext):
    ...
    if call.data == 'dm_withdrawal':
        ...
        if language[4] == 'EN':
            ...
            text = "Select an option to proceed."
        await call.message.answer_photo(photo=photo, caption=text, reply_markup=dm_inline.main_withdraw(language[4]))
    elif call.data == 'dm_confirm_withdrawal':
        wallet = await users.user_data(call.from_user.id)
        async with state.proxy() as data:
        ...
        await call.bot.send_message(
            ...
            f'Режим Демо-версии!'
            f'\n\nПользователь {"@" + username if username is not None else call.from_user.id} '
...
def register(dp: Dispatcher):
    dp.register_callback_query_handler(withdraw_main_menu, text='dm_withdrawal')
    dp.register_callback_query_handler(withdrawal_handler, text='dm_withdrawal_funds')
    dp.register_callback_query_handler(withdrawal_handler_collective, text="dm_withdrawal_500")
    dp.register_callback_query_handler(withdrawal_handler_personal, text="dm_withdrawal_15000")
    dp.register_callback_query_handler(change_wallet_new, text='dm_change_wallet')
    dp.register_callback_query_handler(change_percentage, text='dm_change_percentage')
    dp.register_callback_query_handler(change_percentage_step2, state=DemoChangePercentage.percentage)
    dp.register_message_handler(change_wallet_step2, state=DemoChangeWallet.wallet)
    dp.register_message_handler(handle_amount, state=DemoNewWallet.amount)
    dp.register_callback_query_handler(finish_withdrawal, state=DemoNewWallet.amount)
